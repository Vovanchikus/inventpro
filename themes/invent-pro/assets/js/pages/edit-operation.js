 ...
 * edit-operation.js
 * -----------------
 * Скрипты для страницы редактирования операции
 * Полностью повторяет логику add-operation.js, но для редактирования.
 */
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const formInputsObserver = () => {
        document
            .querySelectorAll("#editOperationForm input")
            .forEach((input) => {
    ...
    // =============================
    const editProducts = JSON.parse(
        localStorage.getItem("editProducts") || "[]"
    );
    if (editProducts.length) {
        editProducts.forEach((product) => addProductToOperation(product));
